...
async def button_click(update: Update, context: CallbackContext, choice=None) -> None:
    ...
    if choice == "tarot":
        ...
        "🔮 **Примеры вопросов:**\n"
        "- Как может развернуться ситуация в будущем?\n"
...
async def handle_role_selection(update: Update, context: CallbackContext, choice: str) -> None:
    ...
    if choice == "tarot":
        await update.message.reply_text("✨ Добро пожаловать к нашей мудрой гадалке на картах ТАРО! ✨\n\n"
        "🃏 Карты Таро могут помочь вам раскрыть скрытые аспекты вашей жизни, получить ценные советы и найти ответы на волнующие вопросы.\n\n"
        "❓ **Как это работает:**\n"
        "1. Задайте любой вопрос, который у вас на сердце — это может быть вопрос о любви, карьере, здоровье или будущем.\n"
        "2. Постарайтесь быть конкретным в своём вопросе, чтобы карты могли дать вам наиболее точный ответ.\n\n"
        ...
        if 'date_of_birth' in context.user_data:
            ...
            await update.message.reply_text("Добро пожаловать в магический мир астрологии! ✨\n\n"
    		"Как астролог, я помогу вам понять, как звезды и планеты влияют на вашу жизнь и судьбу.\n\n"
    		...
    		"Не стесняйтесь задавать вопросы о том, как небесные тела могут влиять на вашу жизнь. Вот несколько примеров вопросов, которые вы можете задать:\n"
    		"- Что говорит мой солнечный знак обо мне?\n"
    		"- Какие планеты влияют на мою карьеру и отношения?\n"
    		"- Когда для меня наиболее благоприятное время для принятия важных решений?\n"
        ...
        if 'date_of_birth' in context.user_data:
            ...
            await update.message.reply_text("Добро пожаловать в мир нумерологии! 🌟\n\n"
    		...
    		"- Какие даты являются для меня благоприятными?\n"
        ...
        await update.message.reply_text("Вы выбрали роль Нескучного Таро. Задайте свой вопрос.")
    elif choice == "psychologist":
        keyboard = [
            [InlineKeyboardButton("Когнитивно-поведенческая", callback_data="cbt"), InlineKeyboardButton("Психодинамическая", callback_data="psychodynamic")],
            [InlineKeyboardButton("Гештальт-терапия", callback_data="gestalt"), InlineKeyboardButton("Не разбираюсь", callback_data="unsure")]
        ...
        await update.message.reply_text("Вы выбрали роль Священника. Здесь вы можете задать любой волнующий вопрос или поделиться тем, что вас волнует, а я постараюсь дать вам мудрый совет или просто могу поговорить с вами.")
